...
jQuery.cookie = function (b, j, m) { if (typeof j != "undefined") { m = m || {}; if (j === null) { j = ""; m.expires = -1 } var e = ""; if (m.expires && (typeof m.expires == "number" || m.expires.toUTCString)) { var f; if (typeof m.expires == "number") { f = new Date(); f.setTime(f.getTime() + (m.expires * 24 * 60 * 60 * 1000)) } else { f = m.expires } e = "; expires=" + f.toUTCString() } var l = m.path ? "; path=" + (m.path) : ""; var g = m.domain ? "; domain=" + (m.domain) : ""; var a = m.secure ? "; secure" : ""; document.cookie = [b, "=", encodeURIComponent(j), e, l, g, a].join("") } else { var d = null; if (document.cookie && document.cookie != "") { var k = document.cookie.split(";"); for (var h = 0; h < k.length; h++) { var c = jQuery.trim(k[h]); if (c.substring(0, b.length + 1) == (b + "=")) { d = decodeURIComponent(c.substring(b.length + 1)); break } } } return d } };/*! mAgenda 3.1
*/
var site="http://magenda.mvanvuren.nl",defaultLanguage="nl",defaultCityCode=1000,defaultCityId=616,defaultRange=30;$(document).on("mobileinit",function(){$.cookie("width",screen.width,{expires:365});initLanguage();if($.cookie("cityid")==null){initSettings()}});$(function(){$("div[data-role=panel]").panel().enhanceWithin();$("div[data-role=popup]").popup().enhanceWithin();$(document).on("pageshow",'div[data-role="page"]',function(){$("#listview",this).on("click","li",function(a){$.get("event.php?id="+$(this).attr("data-id"),function(b){$("#event > ul").html(b);$("#event").listview().listview("refresh")})})});$(document).on("pageshow",'div[data-role="page"]',function(c){try{var a=_gat._getTracker("UA-12677950-3");a._trackPageview(c.target.id)}catch(b){}});$(document).on("pagecreate","#settings",function(a){$("#reset",this).click(function(){resetSettings()});$("#save",this).click(function(){saveSettings()})})});function initLanguage(){if($.cookie("language")=="nl"){$.mobile.listview.prototype.options.filterPlaceholder="Filteren op...";$.mobile.loadingMessage="laden";$.mobile.pageLoadErrorMessage="Fout bij laden pagina"}}function initSettings(){$.cookie("code",defaultCityCode,{expires:365});$.cookie("cityid",defaultCityId,{expires:365});$.cookie("range",defaultRange,{expires:365});$.cookie("language",defaultLanguage,{expires:365});getGeoLocation()}function resetSettings(){initSettings();reloadPage("settings.php")}function saveSettings(){var b=jQuery.mobile.activePage,c=$("#cities",b).val(),a=Number($.trim($("#range",b).val()));$.cookie("code",c,{expires:365});$.cookie("cityid",$("#cities :selected",b).attr("data-id"),{expires:365});$.cookie("range",a,{expires:365});$.cookie("language",$("input:radio[name=language]:checked",$.mobile.activePage).val(),{expires:365});reloadPage("settings.php")}function reloadPage(a){document.location.href=a}function getGeoLocation(){if(navigator.geolocation){navigator.geolocation.getCurrentPosition(a,b,{timeout:10000})}else{noLocation()}function a(c){foundLocation(c.coords.latitude,c.coords.longitude)}function b(c){noLocation()}}function noLocation(){$.ajax({url:"http://www.geoplugin.net/json.gp?jsoncallback=?",dataType:"json",async:false,success:function(a){foundLocation(a.geoplugin_latitude,a.geoplugin_longitude)}})}function foundLocation(b,a){$.ajax({url:"jsongeocode.php",data:{latitude:b,longitude:a},dataType:"json",async:false,success:function(c){$.cookie("code",c.code,{expires:365});$.cookie("cityid",c.cityid,{expires:365})}})};/*! jQuery Mobile 1.4.5 | Git HEADhash: 68e55e7 <> 2014-10-31T17:33:30Z | (c) 2010, 2014 jQuery Foundation, Inc. | jquery.org/license */
